...
from monai.networks.layers import DropPath, Conv, trunc_normal_
from monai.utils import ensure_tuple_rep, optional_import

rearrange, _ = optional_import("einops", name="rearrange")


class SwinUNETR_icl(nn.Module):
    """
    ...
    def __init__(
        ...
        icl_in_chans = (8 * feature_size, 4 * feature_size, 2 * feature_size)
        icl_in_resolutions = (ori_img_size//2**4, ori_img_size//2**3, ori_img_size//2**2)
        icl_num_heads = num_heads[::-1][:3]
        
        # Class Guided Labeled Module initialized
    ...
    def forward(self, x_lab, x_unlab=None, inference=False): # x_labeled, x_unlabeled
        
        # labaled features encode -->
        hidden_states_out_lab = self.swinViT(x_lab, self.normalize)
        enc0_lab = self.encoder1(x_lab)
        enc1_lab = self.encoder2(hidden_states_out_lab[0]) # x0_out: BS,48,48,48,48
        enc2_lab = self.encoder3(hidden_states_out_lab[1]) # x1_out: BS,96,24,24,24
        enc3_lab = self.encoder4(hidden_states_out_lab[2]) # x2_out: BS,192,12,12,12
        dec4_lab = self.encoder10(hidden_states_out_lab[4]) # x4_out: BS,768,3,3,3

        # labeled features decode -->
        dec3_lab = self.decoder5(dec4_lab, hidden_states_out_lab[3]) # x3_out: BS,384,6,6,6
        dec2_lab = self.decoder4(dec3_lab, enc3_lab)
        dec1_lab = self.decoder3(dec2_lab, enc2_lab)
        dec0_lab = self.decoder2(dec1_lab, enc1_lab)
        out_lab = self.decoder1(dec0_lab, enc0_lab)

        logits_lab = self.out(out_lab)

        if inference:
            return logits_lab
        else:
            # unlabaled features encode -->
            hidden_states_out_unlab = self.swinViT(x_unlab, self.normalize)
            enc0_unlab = self.encoder1(x_unlab)
            enc1_unlab = self.encoder2(hidden_states_out_unlab[0])
            enc2_unlab = self.encoder3(hidden_states_out_unlab[1])
            enc3_unlab = self.encoder4(hidden_states_out_unlab[2])
            dec4_unlab = self.encoder10(hidden_states_out_unlab[4])

            # unlabeled features decode -->
            dec3_unlab = self.decoder5(dec4_unlab, hidden_states_out_unlab[3]) # x3_out: BS,384,6,6,6
            dec2_unlab = self.decoder4(dec3_unlab, enc3_unlab)
            dec1_unlab = self.decoder3(dec2_unlab, enc2_unlab)
            dec0_unlab = self.decoder2(dec1_unlab, enc1_unlab)
            out_unlab = self.decoder1(dec0_unlab, enc0_unlab)

        
            logits_unlab = self.out(out_unlab)

            feats_lab = [dec3_lab, dec2_lab, dec1_lab, dec0_lab]
            feats_unlab = [dec3_unlab, dec2_unlab, dec1_unlab, dec0_unlab]
            ...
            return logits_lab, logits_unlab, feat_Maps_lab, feat_Maps_unlab, feat_Maps_consis
...
class InherentConsistent(nn.Module):
    def __init__(
        ...
        self.guide_Q = nn.Parameter(torch.zeros(1, num_classes, in_chans[0]))
    ...
    def forward(self, feats, guided_Q=None, modal='labeled'):        
        ...
        if modal == 'labeled':
            next_guided_Q = self.guide_Q.expand(BS, -1, -1)
...
class SwinTransformer(nn.Module):
    ...
    def forward(self, x, normalize=True):
        # x: (BS,1,96,96,96)
        # x0: (BS,48,48,48,48) -> x0_out: (BS,48,48,48,48)
        # x1: (BS,24,24,24,96) -> x1_out: (BS,24,24,24,96)
        # x2: (BS,192,12,12,12) -> x2_out: (BS,192,12,12,12)
        # x3: (BS,384,6,6,6) -> # x3_out: (BS,384,6,6,6)
        # x4: (BS,768,3,3,3) -> # x3_out: (BS,768,3,3,3)

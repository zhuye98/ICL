...
class SwinTransformerSys(nn.Module):
    ...
    def forward(self, x_lab, x_unlab=None, inference=False):
        x_lab, x_lab_downsample = self.forward_features(x_lab)
        x_lab_last, feats_lab = self.forward_up_features(x_lab, x_lab_downsample)
        output_lab = self.up_x4(x_lab_last)
        
        if inference:
            return output_lab

        else:
            x_unlab, x_unlab_downsample = self.forward_features(x_unlab)
            x_unlab_last, feats_unlab = self.forward_up_features(x_unlab, x_unlab_downsample)
            output_unlab = self.up_x4(x_unlab_last)
        
            return output_lab, output_unlab, feats_lab, feats_unlab

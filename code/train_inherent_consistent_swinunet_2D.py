...
parser.add_argument('--model', type=str,
                    default='icl_swinunet', help='model_name')
...
def train(args, snapshot_path):
    ...
    aux_loss = losses.AuxLoss(num_classes)
    pse_loss = losses.PseudoSoftLoss(num_classes)
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            # NOTE: loss 'weight superparameters
            ...
            logging.info('iteration %d : loss : %f, loss_ce: %f, loss_aux: %f, loss_pse: %f, loss_aux_consis: %f' % (iter_num, loss.item(), loss_ce.item(), loss_aux.item(), loss_pse.item(), 50*loss_aux_consis.item()))
            ...
            if iter_num > 0 and (iter_num % 200) == 0:
                ...
                for i_batch, sampled_batch in enumerate(valloader):
                    metric_i = test_single_volume_ours(sampled_batch["image"], sampled_batch["label"], model, writer, iter_num, classes=num_classes, patch_size=args.patch_size)
